<template>
    <div class="app-container">
        ...
        <el-table v-loading="loading" :data="data" size="small" border style="width: 100%;">
            <el-table-column prop="examName" label="试卷名" />
            <el-table-column prop="examId" label="试卷id" />
            <el-table-column prop="userId" label="用户id" />
            <el-table-column prop="userName" label="用户名" />
            <el-table-column prop="score" label="成绩" />
            <el-table-column prop="average" label="答对率" />
...
export default {
  ...
  created() {
    if (checkPermission(["PERMISSION_ALL"])) {
      var userInfo = JSON.parse(localStorage.getItem("userInfo"));
       var sql= examOpt.find.replace('?',userInfo.id)
      this.$http
        .post("action", {
          sql: sql
        })
        .then(res => {
           this.examIdList=res.data.map(item=>{
               return item.id
           }) 
          this.$nextTick(() => {
            this.init();
  ...
  methods: {
    ...
    beforeInit() {
      ...
      if (checkPermission(["PERMISSION_ALL"])) {
          this.params.examIdJson=encodeURI(JSON.stringify(this.examIdList)) 
      }
