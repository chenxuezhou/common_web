        ...
        <el-table v-loading="loading" :data="data" size="small" border style="width: 100%;">
            ...
            <el-table-column label="操作" width="150px" align="center">
                <template slot-scope="scope">
                    ...
                    <el-popover v-if="checkPermission(['ADMIN','USER_ALL','USER_DELETE'])" :ref="scope.row.id" placement="top" width="180">
                        <p>确定删除本条数据吗？</p>
  ...
  methods: {
    ...
    beforeInit() {
      ...
      if (enabled !== "" && enabled !== null) {
        this.and["enabled"] = enabled;
      }
      this.or = {};
      //     if (enabled !== "" && enabled !== null) {
      //     this.or["enabled"] = enabled;
      //   }
      return true;
    },
    subDelete(id) {
    ...
    getRoles() {
      var sql = base.getAll("role");
      this.$http.post("action", { sql: sql }).then(res => {
        this.roles = res.data.map(item=>{return {id:item.id,label:item.name}});
      });
    }
  }
};
